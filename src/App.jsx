...
const adminLogout = async () => {
  ...
  const placeOrder = async (customer, linesToOrder = cartLines, totals = { subtotal, deliveryFee, total }, clearCart = true) => {
    ...
    // Email is deliberately triggered only after the database insert succeeds.
    const { error: emailError } = await supabase.functions.invoke(ORDER_EMAIL_FUNCTION, {
      body: { order: savedOrder, recipient: "sabeenarain.0@gmail.com" },
    });
    if (emailError) {
